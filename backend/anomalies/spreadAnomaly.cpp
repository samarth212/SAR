...
double averageSpreadOfRecentQuotes(const std::string& symbol, const std::unordered_map<std::string, SymbolState>& bySymbol){
...
std::optional<Anomaly> detectSpreadAnomaly(const std::string& symbol, const std::unordered_map<std::string, SymbolState>& bySymbol, double k){
    ...
    if(newSpread > avgSpread + (k * stdev)){
        ...
        newAnomaly.note =
        "Upward spread anomaly: " + symbol +
        " has a bid-ask spread of " + std::to_string(newSpread) +
        ", which is above the recent average " + std::to_string(avgSpread) +
        " by " + std::to_string(newSpread - avgSpread) +
        " (" + std::to_string(newAnomaly.zscore) + " standard deviations). "
        "This suggests liquidity is thinner than usual and prices may be less stable, which can happen during uncertainty, low activity, or around fast-moving news.";
        ...
        newAnomaly.note =
        "Downward spread anomaly: " + symbol +
        " has a bid-ask spread of " + std::to_string(newSpread) +
        ", which is below the recent average " + std::to_string(avgSpread) +
        " by " + std::to_string(avgSpread - newSpread) +
        " (" + std::to_string(-newAnomaly.zscore) + " standard deviations). "
        "This suggests unusually tight liquidity and smoother trading conditions than normal, which can happen when many buyers and sellers are active at the same time.";
